...
## Android TV Accessibility Service

Google TV and some Android TV devices intercept the `Home` and `Menu` buttons on controllers before apps can see them.

Cannoli includes an optional accessibility service that works around this. When enabled, it intercepts these buttons only while Cannoli is in the foreground and passes them through so they work as expected.
