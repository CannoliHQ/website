...
??? question "How do I make or edit a collection?"
    Select a game and press `Start`. In the context menu pick `Manage Collections`.
    
    To make a new collection press `X`. Give the collection a name and then press `Start` to save it.

    The checkboxes that appear in front of the collection names show which collections the game belongs to.
