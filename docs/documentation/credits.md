## Credits

Cannoli only exists because of the incredible work of others.

Their generosity cannot be overstated.

Again a special thanks to Shaun Inman for creating MinUI. 

I can only hope that Cannoli ends up half as good.

### Cannoli Branding

The smiling cannoli logo was made by Abraham McCampbell (Tenseiren)!

You can find their work and contact information over on [ArtStation](https://tenseiren.artstation.com).

Abraham, thank you for being an absolute pleasure to work with and for making Cannoli look like a real project!

### Cores

| Name                    | License                                                                                                     |
|-------------------------|-------------------------------------------------------------------------------------------------------------|
| FBNeo                   | [Non-commercial](https://github.com/CannoliHQ/cannoli/blob/main/licenses/cores/fbneo-LICENSE.txt)           |
| Gambatte                | [GPLv2](https://github.com/CannoliHQ/cannoli/blob/main/licenses/cores/gambatte-GPLv2.txt)                   |
| Genesis Plus GX         | [Non-commercial](https://github.com/CannoliHQ/cannoli/blob/main/licenses/cores/genesis-plus-gx-LICENSE.txt) |
| Handy                   | [Zlib](https://github.com/CannoliHQ/cannoli/blob/main/licenses/cores/handy-ZLIB.txt)                        |
| MAME 2003-Plus          | [MAME](https://github.com/CannoliHQ/cannoli/blob/main/licenses/cores/mame2003-plus-LICENSE.txt)             |
| Mednafen Neo Geo Pocket | [GPLv2](https://github.com/CannoliHQ/cannoli/blob/main/licenses/cores/mednafen-ngp-GPLv2.txt)               |
| Mednafen PCE FAST       | [GPLv2](https://github.com/CannoliHQ/cannoli/blob/main/licenses/cores/mednafen-pce-fast-GPLv2.txt)          |
| Mednafen Virtual Boy    | [GPLv2](https://github.com/CannoliHQ/cannoli/blob/main/licenses/cores/mednafen-vb-GPLv2.txt)                |
| Mednafen WonderSwan     | [GPLv2](https://github.com/CannoliHQ/cannoli/blob/main/licenses/cores/mednafen-wswan-GPLv2.txt)             |
| mGBA                    | [MPLv2.0](https://github.com/CannoliHQ/cannoli/blob/main/licenses/cores/mgba-MPLv2.txt)                     |
| Nestopia                | [GPLv2](https://github.com/CannoliHQ/cannoli/blob/main/licenses/cores/nestopia-GPLv2.txt)                   |
| PCSX ReARMed            | [GPLv2](https://github.com/CannoliHQ/cannoli/blob/main/licenses/cores/pcsx-rearmed-GPLv2.txt)               |
| PokeMini                | [GPLv3](https://github.com/CannoliHQ/cannoli/blob/main/licenses/cores/pokemini-GPLv3.txt)                   |
| ProSystem               | [GPLv2](https://github.com/CannoliHQ/cannoli/blob/main/licenses/cores/prosystem-GPLv2.txt)                  |
| Snes9x                  | [Non-commercial](https://github.com/CannoliHQ/cannoli/blob/main/licenses/cores/snes9x-LICENSE.txt)          |
| Stella                  | [GPLv2](https://github.com/CannoliHQ/cannoli/blob/main/licenses/cores/stella-GPLv2.txt)                     |
| SwanStation             | [GPLv3](https://github.com/CannoliHQ/cannoli/blob/main/licenses/cores/swanstation-GPLv3.txt)                |

### Shaders

| Name                        | What                  | License                                                                                                            |
|-----------------------------|-----------------------|--------------------------------------------------------------------------------------------------------------------|
| EasyMode                    | crt-aperture          | [GPL](https://github.com/CannoliHQ/cannoli/blob/main/licenses/shaders/crt-aperture-GPL.txt)                        |
| Cannoli (based on EasyMode) | crt-cannoli           | [GPL](https://github.com/CannoliHQ/cannoli/blob/main/licenses/shaders/crt-cannoli-GPL.txt)                         |
| EasyMode                    | crt-easymode          | [GPL](https://github.com/CannoliHQ/cannoli/blob/main/licenses/shaders/crt-easymode-GPL.txt)                        |
| cgwg / Themaister / DOLLS   | crt-geom              | [GPLv2](https://github.com/CannoliHQ/cannoli/blob/main/licenses/shaders/crt-geom-GPLv2.txt)                        |
| Timothy Lottes / hunterk    | crt-lottes-fast       | [Public domain](https://github.com/CannoliHQ/cannoli/blob/main/licenses/shaders/crt-lottes-fast-PUBLIC-DOMAIN.txt) |
| Themaister                  | dot                   | [Public domain](https://github.com/CannoliHQ/cannoli/blob/main/licenses/shaders/dot-PUBLIC-DOMAIN.txt)             |
| Gigaherz                    | lcd3x                 | [Public domain](https://github.com/CannoliHQ/cannoli/blob/main/licenses/shaders/lcd3x-PUBLIC-DOMAIN.txt)           |
| hunterk                     | scanline              | [Public domain](https://github.com/CannoliHQ/cannoli/blob/main/licenses/shaders/scanline-fract-PUBLIC-DOMAIN.txt)  |
| rsn8887                     | sharp-bilinear-simple | [Public domain](https://github.com/CannoliHQ/cannoli/blob/main/licenses/shaders/sharp-bilinear-PUBLIC-DOMAIN.txt)  |
| SoltanGris42                | zfast-crt             | [GPLv2](https://github.com/CannoliHQ/cannoli/blob/main/licenses/shaders/zfast-crt-GPLv2.txt)                       |
| SoltanGris42                | zfast-lcd             | [GPLv2](https://github.com/CannoliHQ/cannoli/blob/main/licenses/shaders/zfast-lcd-GPLv2.txt)                       |

### Fonts

| Name             | What              | License                                                                                            |
|------------------|-------------------|----------------------------------------------------------------------------------------------------|
| M+ Fonts Project | M PLUS 1 Code     | [OFL](https://github.com/CannoliHQ/cannoli/blob/main/licenses/fonts/mplus1code-OFL.txt)            |
| BPreplay         | BPreplay Unhinted | [OFL](https://github.com/CannoliHQ/cannoli/blob/main/licenses/fonts/BPreplayBold-unhinted-OFL.txt) |
| Nerd Fonts       | NerdSymbols       | [OFL](https://github.com/CannoliHQ/cannoli/blob/main/licenses/fonts/nerdsymbols-OFL.txt)           |

### Libraries

| Name                      | What               | License                                                                                                   |
|---------------------------|--------------------|-----------------------------------------------------------------------------------------------------------|
| Apache Commons Compress   | Archive Extraction | [Apache 2.0](https://github.com/CannoliHQ/cannoli/blob/main/licenses/APACHE-2.0.txt)                      |
| PdfiumAndroid (io.legere) | PDF Renderer       | [Apache 2.0](https://github.com/CannoliHQ/cannoli/blob/main/licenses/libraries/pdfiumandroid-APACHE2.txt) |
| XZ for Java               | 7z Decompression   | [Public domain](https://tukaani.org/xz/java.html)                                                         |
| ZXing                     | QR Code Library    | [Apache 2.0](https://github.com/CannoliHQ/cannoli/blob/main/licenses/libraries/zxing-APACHE2.txt)         |
